...
const MultiChartDashboard = () => {
  ...
  useLayoutEffect(() => {
    ...
    // 👇 Robust root creator — handles disposed/undefined safely
    const createRoot = (divId) => {
      try {
        am5.array.each(am5.registry.rootElements, (r) => {
          // skip if r is disposed or has no DOM
          if (!r || r.isDisposed() || !r.dom) return;
          if (r.dom.id === divId) {
            try {
              r.dispose();
            } catch {
        ...
        /* ignore any amCharts internal errors */
      }
    ...
    const initCharts = () => {
      // 1️⃣ MAP CHART
      const mapRoot = createRoot("mapdiv");
      const mapChart = mapRoot.container.children.push(
        am5map.MapChart.new(mapRoot, { projection: am5map.geoNaturalEarth1() })
      );
      ...
      // 2️⃣ COLUMN CHART
      const colRoot = createRoot("columndiv");
      const colChart = colRoot.container.children.push(
        am5xy.XYChart.new(colRoot, { panX: false, panY: false })
      );

      const colx = colChart.xAxes.push(
        am5xy.CategoryAxis.new(colRoot, {
          categoryField: "date",
          renderer: am5xy.AxisRendererX.new(colRoot, {}),
        })
      );
      const coly = colChart.yAxes.push(
        am5xy.ValueAxis.new(colRoot, {
          renderer: am5xy.AxisRendererY.new(colRoot, {}),
        })
      );

      const colSeries = colChart.series.push(
        am5xy.ColumnSeries.new(colRoot, {
          ...
          fill: mainColor,
          stroke: mainColor,
          tooltip: am5.Tooltip.new(colRoot, { labelText: "{valueY}" }),
        })
      );
      ...
      colx.data.setAll(colData);
      colSeries.data.setAll(colData);

      // 3️⃣ PIE CHART
      const pieRoot = createRoot("piediv");
      const pieChart = pieRoot.container.children.push(
        am5percent.PieChart.new(pieRoot, { innerRadius: am5.percent(70) })
      );
      ...
      // 4️⃣ RADAR CHART
      const radarRoot = createRoot("gaugediv");
      const radar = radarRoot.container.children.push(
      ...
      ry.data.setAll(rData);

      const bg = radar.series.push(
        am5radar.RadarColumnSeries.new(radarRoot, {
          xAxis: rx,
          yAxis: ry,
          valueXField: "full",
          categoryYField: "category",
      ...
      bg.data.setAll(rData);

      const fg = radar.series.push(
        am5radar.RadarColumnSeries.new(radarRoot, {
          xAxis: rx,
          yAxis: ry,
          valueXField: "value",
      ...
      fg.data.setAll(rData);

      // 5️⃣ LINE CHART
      const lineRoot = createRoot("linediv");
      const lineChart = lineRoot.container.children.push(
        am5xy.XYChart.new(lineRoot, { layout: lineRoot.verticalLayout })
      );
      ...
      const lx = lineChart.xAxes.push(
        am5xy.CategoryAxis.new(lineRoot, {
          categoryField: "year",
          renderer: am5xy.AxisRendererX.new(lineRoot, {}),
        })
      );
      lx.data.setAll(lineData);

      const ly = lineChart.yAxes.push(
        am5xy.ValueAxis.new(lineRoot, { min: 0, extraMax: 0.1 })
      );

      const addLine = (name, field, color) => {
        const s = lineChart.series.push(
          am5xy.LineSeries.new(lineRoot, {
            ...
            stroke: color,
            fill: color,
            tooltip: am5.Tooltip.new(lineRoot, { labelText: "{name}: {valueY}" }),
          })
        );
        s.data.setAll(lineData);
        s.strokes.template.setAll({ strokeWidth: 3 });
      };

      addLine("Income", "income", secondaryColor);
      addLine("Expenses", "expenses", mainColor);
      lineChart.set("cursor", am5xy.XYCursor.new(lineRoot, {}));
    };
  ...
  return (
    <div className="w-full grid grid-cols-2 gap-6 p-4">
      ...
      <div id="columndiv" style={{ width: "100%", height: "400px" }} />
      <div id="piediv" style={{ width: "100%", height: "400px" }} />
      <div id="gaugediv" style={{ width: "100%", height: "400px" }} />
      <div id="linediv" style={{ width: "100%", height: "400px" }} />
    </div>
  );
};
